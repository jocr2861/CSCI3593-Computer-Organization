...
// ========== CODASIP ==========
// LLVM has officially only 4.2.1 compatibility, but it is almost compatible
// with later versions, so there's no reason not to use the available builtins:
#if __GNUC_PREREQ (4, 4) || (__has_builtin(__builtin_fpclassify) \
                          && __has_builtin(__builtin_isfinite) \
                          && __has_builtin(__builtin_isinf_sign) \
                          && __has_builtin(__builtin_isnan) \
                          && __has_builtin(__builtin_isnormal))
// =============================
#if 0   // replaced by Codasip above
