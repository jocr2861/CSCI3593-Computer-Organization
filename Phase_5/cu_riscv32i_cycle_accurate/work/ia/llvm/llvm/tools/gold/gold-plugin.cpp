...
static std::set<std::string> SymTable;
//====================================
...
ld_plugin_status onload(ld_plugin_tv *tv) {
  ...
  //============================== CODASIP ==============================
  // always create output bc file, clang driver expects the file to exist
  std::error_code EC;
  raw_fd_ostream OS(output_name + ".bc", EC, sys::fs::OpenFlags::F_None);
  if (EC) {
    message(LDPL_ERROR, "Failed to open the bitcode intermediate file.");
    return LDPS_ERR;
  }
  //=====================================================================
...
static void addModule(LTO &Lto, claimed_file &F, const void *View,
  ...
  for (ld_plugin_symbol &Sym : F.syms) {
    ...
    case LDPR_PREEMPTED_REG:
      //============ CODASIP ============
      // fixes potential bug in ld when --whole-archive is used on a
      // library that defines a symbol already defined in app
      // ld will claim that the library version should be used
      // even when the user version isn't weak
      if (Sym.def != LDPK_WEAKDEF) {
    ...
    case LDPR_PREVAILING_DEF_IRONLY:
      //========================= CODASIP ==========================
      // if symbol is defined multiple times, pick the last definition
      // can happen in case application defines symbol already defined
      // in a library or when multiple libraries define the same symbol
      for (auto it = SymTable.begin(); it != SymTable.end(); it++) {
    	if (StringRef(*it).equals(Sym.name)) {
          R.Prevailing = !isUndefined(Sym);
          if (R.Prevailing) {
            SymTable.erase(it);
          }
...
static std::unique_ptr<LTO> createLTO(IndexWriteCallback OnIndexWrite,
  ...
  //======================== CODASIP ============================
  // save bitcode after optimizations
  check(Conf.CodasipSaveOptBc(output_name));
  //=============================================================
...
static std::vector<std::pair<SmallString<128>, bool>> runLTO() {
  ...
  //============== CODASIP ===============
  // get all symbols defined in all modules
  for (claimed_file &F : Modules) {
    for (ld_plugin_symbol &Sym : F.syms) {
      SymTable.insert(Sym.name);
    }
  }
  //======================================
  for (claimed_file &F : Modules) {
    ...
    //=============== CODASIP ==================
    // disabled the following check to fix tests with newlib-micro
    //==========================================
#if 0
