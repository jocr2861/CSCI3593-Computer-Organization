...
#ifndef OS_FILESYSTEML_FILESYSTEM_H_
#define OS_FILESYSTEML_FILESYSTEM_H_
 ...
 * \param[out] fileSize On success will contain size of file.
 * \return     true, if size of file on \p path is stored into \p size,
 *             false on error.
 */
bool FileSize(const char* path, unsigned int& fileSize);
/**
 * \brief  Creates directory.
 * \param  dir Path of directory to be created. Path is normalized before creating directory.
 ...
 */
bool CreateDirectory(const Path& dir, const bool recursive = true);
/**
...
public:
     ...
     * \param  path String to be appended to internally stored path.
     * \return reference to same object.
     * \note   Example: <tt>foo/bar Concat test -> /foo/bartest</tt>
     */
    Path& Concat(const char* path);
	 ...
	 *         path object.
	 * \param  right Path to be added to currently stored path.
     ...
     *         \p path to current path. Doesn't modify current
     *         object.
     * \param  right Path to be concatenated to currently stored path.
     * \return New path object.
     ...
     */
    bool GetFileSize(unsigned int& size) const
    {
        return FileSize(m_path.c_str(), size);
    }
    /**
	 * \brief Clears path.
	 */
     ...
     *         successful copy current path is changed to destination path.
 ...
 */
inline bool operator<(Path const& l, Path const& r)
{
 ...
 */
class DirectoryIterator : public std::iterator<std::forward_iterator_tag,   // Forward iterator type
                              Path,                                         // Iterator type
...
#endif /* OS_FILESYSTEML_FILESYSTEM_H_ */
