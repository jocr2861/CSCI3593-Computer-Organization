...
/// Used to supress propagation of warnings coming out of LLVM project
#ifndef _MSVC
#    define CODASIP_LLVM_INCLUDE_BEGIN \
        _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wunused-parameter\"")
#    define CODASIP_LLVM_INCLUDE_END _Pragma("GCC diagnostic pop")
#else
#    define CODASIP_LLVM_INCLUDE_BEGIN \
        __pragma(warning(push)) __pragma(warning(disable : 4244 4624 4146 4141 4291))
#    define CODASIP_LLVM_INCLUDE_END __pragma(warning(pop))
#endif
...
#else
    return ((((uint32_t)(n)&0x000000FF) << 24) | (((uint32_t)(n)&0x0000FF00) << 8) |
        (((uint32_t)(n) >> 8) & 0x0000FF00) | (((uint32_t)(n) >> 24) & 0x000000FF));
#endif
}
...
#else
    return (((uint64_t)(n) << 56) | (((uint64_t)(n) << 40) & 0X00FF000000000000ULL) |
        ...
        (((uint64_t)(n) >> 40) & 0X000000000000FF00ULL) | ((uint64_t)(n) >> 56));
#endif
}
