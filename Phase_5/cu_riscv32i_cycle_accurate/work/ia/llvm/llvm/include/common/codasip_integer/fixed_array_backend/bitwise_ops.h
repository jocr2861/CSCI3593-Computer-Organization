...
#ifndef COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_BITWISE_OPS_H_
#define COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_BITWISE_OPS_H_
 ...
 */
template<class Binary, unsigned LHS_SIZE, unsigned RHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE void DoBitwiseOp(T1* lhs, const T2* rhs)
{
 ...
 */
template<class Binary, unsigned LHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((sizeof(T1) >= sizeof(T2)), void) DoBitwiseOp(T1* lhs, const T2 rhs)
{
 ...
 */
template<unsigned LHS_SIZE, typename T>
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF((LHS_SIZE == 1), void)
    DoBitwiseShiftRightCompound(T* lhs, const unsigned rhs)
{
...
template<unsigned LHS_SIZE, typename T>
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((LHS_SIZE > 1), void) DoBitwiseShiftRightCompound(T* lhs, const unsigned rhs)
{
    enum : unsigned
    {
        T_BITS = native_int_bitwidth_helper<T>::value
    };
    ...
    {   // moving aligned elements, e.g. uint128 >> 32
        ...
        std::memset(lhs + LHS_SIZE - offset, GetSignExtension(lhs[LHS_SIZE - 1]), offset * sizeof(T));
    }
    else
    {   // unaligned shift
        ...
        {
            lhs[ii - offset] = ShiftRightSingleRoundAnaligned(lhs[ii + 1], lhs[ii], remLeft, remRight);
        }
        const T sign = GetSignExtension(lhs[LHS_SIZE - 1]);
        lhs[LHS_SIZE - 1 - offset] =
            ShiftRightSingleRoundAnaligned(sign, lhs[LHS_SIZE - 1], remLeft, remRight);
        // fill highest bits with sign
...
template<unsigned LHS_SIZE, typename T>
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((LHS_SIZE == 1), void) DoBitwiseShiftLeftCompound(T* lhs, const unsigned rhs)
{
    if (rhs >= native_int_bitwidth_helper<T>::value)
    {
        lhs[0] = 0;
...
template<unsigned LHS_SIZE, typename T>
_CODASIP_INT_INLINE void DoBitwiseShiftLeftCompound(T* result, const T* lhs, const unsigned rhs)
{
    enum : int
    {
        T_BITS = native_int_bitwidth_helper<T>::value
...
#endif   // COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_BITWISE_OPS_H_
