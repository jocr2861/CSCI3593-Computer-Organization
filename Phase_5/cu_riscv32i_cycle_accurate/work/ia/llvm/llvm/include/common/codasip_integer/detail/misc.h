...
#ifndef COMMON_CODASIP_INTEGER_DETAIL_MISC_H_
#define COMMON_CODASIP_INTEGER_DETAIL_MISC_H_
...
template<unsigned _BITS, bool _SIGN, bool IS_VALID = is_trivial_fixed_int<_BITS>::value>
...
struct underlying_type_helper<_BITS, _SIGN, true>
{
    ...
    typedef typename std::conditional<_BITS <= std::numeric_limits<uint8_t>::digits,
        int8_type,
        typename std::conditional<_BITS <= std::numeric_limits<uint16_t>::digits,
            int16_type,
            typename std::conditional<_BITS <= std::numeric_limits<uint32_t>::digits, int32_type, int64_type>::type>::type>::type
        type;
};

template<unsigned _BITS, bool _SIGN, unsigned UNDERLYING_BITS = native_int_bitwidth_helper<uint8_t>::value>
 ...
 */
template<template<unsigned, bool> class Number, unsigned LHS_BITS, bool LHS_SIGN, unsigned RHS_BITS, bool RHS_SIGN>
struct BinaryOperatorHelper
{
    /// bit-width of the integer number
    static const unsigned BIT_WIDTH = return_type_helper<LHS_BITS, LHS_SIGN, RHS_BITS, RHS_SIGN>::BINARY_W;
    /// sign of the integer number
    static const bool IS_SIGNED = return_type_helper<LHS_BITS, LHS_SIGN, RHS_BITS, RHS_SIGN>::BINARY_S;
    /// resulting type
...
template<template<unsigned, bool> class Number, unsigned LHS_BITS, bool LHS_SIGN, typename T>
struct BinaryResultNative
  : public BinaryOperatorHelper<Number, LHS_BITS, LHS_SIGN, native_int_bitwidth_helper<T>::value, std::numeric_limits<T>::is_signed>
{};
...
struct use_direct_native_int_comparator
{
    static const bool value = (LHS_SIGN == RHS_SIGN) ||
        (is_native_type_bitwidth<LHS_BITS>::value && is_native_type_bitwidth<RHS_BITS>::value);
};
 ...
 */
inline void DetectIntegerFormatFromString(bool& isNeg, unsigned& radix, size_t& n, const char*& str)
{
    if (n && (*str == '-'))
    {
        isNeg = true;
...
inline std::string ToHexadecimalStr(const T value, const char a)
{
    ...
    {
        typename std::string::value_type c = '0' +
            static_cast<typename std::string::value_type>(uv & MASK);
        if (c > '9')
        {   // upper or lower case
            c += a - '9' - 1;
        }
    ...
    {
        typename std::string::value_type c = '0' +
            static_cast<typename std::string::value_type>(uv & MSB_MASK);
        if (c > '9')
        {
            c += a - '9';
        }
...
inline std::string GetNumberPrefix(const std::ios_base::fmtflags flags, const bool isNeg)
{
    std::string result;
...
_CODASIP_INT_INLINE T GetUnaryNegation(const T value)
{
    static_assert(std::is_integral<T>::value,
        "Unexpected usage, should be used with native integer types.");
...
#endif   // COMMON_CODASIP_INTEGER_DETAIL_MISC_H_
