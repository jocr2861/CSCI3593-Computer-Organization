...
#ifndef COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_ASSIGN_H_
#define COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_ASSIGN_H_
...
{
    _CODASIP_INT_INLINE S operator()(const V*) const
    {
        return 0;
    }
};
...
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((LHS_SIZE == 1) || ((LHS_SIZE > 1) && (sizeof(S) >= (sizeof(V) * LHS_SIZE))),
    void) DoToVector(V* vec, const S scalar)
...
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((LHS_SIZE <= RHS_SIZE), void) DoAssign(T1* lhs, const T2* rhs)
{
...
{   // do standard assignment of the common part
    ...
    std::memset(lhs + RHS_SIZE, GetSignExtension(rhs[RHS_SIZE - 1]), (LHS_SIZE - RHS_SIZE) * sizeof(T1));
}
...
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF_POD(T2, void) DoAssign(T1* lhs, const T2 rhs)
{
 ...
 */
template<typename S, unsigned LHS_SIZE, typename V>
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF((LHS_SIZE == 1) || (sizeof(S) <= sizeof(V)), S)
...
template<typename S, unsigned LHS_SIZE, typename V>
_CODASIP_INT_INLINE
    _CODASIP_INT_ENABLE_IF((LHS_SIZE > 1) && (sizeof(S) > sizeof(V)), S) DoToScalar(const V* array)
...
#endif   // COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_ASSIGN_H_
