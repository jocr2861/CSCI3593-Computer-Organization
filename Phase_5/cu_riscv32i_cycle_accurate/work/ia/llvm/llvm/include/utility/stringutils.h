...
#ifndef UTILITY_STRINGUTILS_H_
#define UTILITY_STRINGUTILS_H_
    ...
    const bool bWholeWord = false)
{
    ...
    while (kk != std::string::npos)
    {
        ...
        {
            if ((kk == 0 ||
                    (!isalnum(sContent[kk - 1]) && sContent[kk - 1] != '_' && sContent[kk - 1] != '-')) &&
                !isalnum(sContent[kk + sOld.length()]) && sContent[kk + sOld.length()] != '_' &&
                sContent[kk + sOld.length()] != '-')
            {
                replace = true;
            }
        }
 ...
 */
inline std::string& Substitute(std::string& str, const std::string& sOld, const std::string& sNew)
{
    std::string::size_type pos = 0;
    while (((pos = str.find(sOld, pos)) != std::string::npos) && (pos < str.size()))
    {
...
inline std::string& Substitute(std::string& str, const char sOld, const std::string& sNew)
{
    return Substitute(str, std::string(1, sOld), sNew);
}

inline std::string& Substitute(std::string& str, const char sOld, const char sNew)
{
    std::replace(str.begin(), str.end(), sOld, sNew);
    return str;
}

inline std::string Substitute(const std::string& str, const char sOld, const char sNew)
{
    std::string copy(str);
...
{
    typename std::insert_iterator<ContainerType> inserter(container, container.end());
...
inline TokensContainerType&
SplitTokens(TokensContainerType& tokens, PositionsContainerType& tokenPositions, const std::string& source)
{
    typename std::insert_iterator<TokensContainerType> tokensInserter(tokens, tokens.end());
    typename std::insert_iterator<PositionsContainerType> positionsInserter(tokenPositions,
        tokenPositions.end());
 ...
 *          if both the arguments are non-empty.
 ...
 */
inline std::string
Concatenate(const std::string& first, const std::string& second, const std::string& del)
{
...
inline std::string
ConcatenateNames(InputIterator begin, InputIterator end, const std::string& delimiter)
{
    std::string value;
 ...
 */
template<class ContainerType>
inline std::string ConcatenateNames(const ContainerType& container, const std::string& delimiter)
{
 ...
 */
inline std::string
RemovePrefixSuffix(const std::string& input, const std::string& prefix, const std::string& suffix)
{
...
/**
 *  \class  codasip::detail::OutputFunctorHelper
 *  \brief  Helper class used to write data return by the unary function object
 *          to the output stream.
 */
struct OutputFunctorHelper
{
public:
    /**
     *  \brief  Calls the output functor for given iterator value and writes the
     *          return value of the output functor to standard output stream.
     *  \param  out reference to standard output stream.
     *  \param  it iterator to the source value.
     *  \param  output unary functor that will be used to write out the item to
     *          the output stream.
     */
    template<class InputIterator, class OutputFunctor>
    static void Write(std::ostream& out, InputIterator it, OutputFunctor&& output)
    {
        DoWrite<std::is_same<void, decltype(output(*it))>::value>(out,
            it,
            std::forward<OutputFunctor>(output));
    }

private:
    template<bool IsVoid, class InputIterator, class OutputFunctor>
    static typename std::enable_if<IsVoid>::type
    DoWrite(std::ostream&, InputIterator it, OutputFunctor&& output)
    {   // no return value to be written, the functor must have its own stream
        output(*it);
    }
    template<bool IsVoid, class InputIterator, class OutputFunctor>
    static typename std::enable_if<!IsVoid>::type
    DoWrite(std::ostream& out, InputIterator it, OutputFunctor&& output)
    {   // write return value of the output functor
        out << output(*it);
    }
};
...
/**
 *  \{
 *  \brief  Write out list of items given by iterators. By default, items
 *          are separated by commas. After the last item, there is no delimiter.
 *          The list is generated to given output stream. Standard output stream
 *          operator << is used for output.
 *  \param  out target output stream
 *  \param  first iterator to the source container, first item to write.
 *  \param  last iterator that refers to the past-the-end, the iterator won't
 *          be generated.
 *  \param  delimiter separates two items, default value is a comma with one
 *          space character.
 */
template<class InputIterator>
inline void
WriteList(std::ostream& out, InputIterator first, InputIterator last, const char* delimiter = ", ")
{
    while (first != last)
    {
        out << *first;
...
template<class InputIterator>
inline void
WriteList(std::ostream& out, InputIterator first, InputIterator last, const std::string& delimiter)
{
    WriteList(out, first, last, delimiter.c_str());
}
/**
 *  \} \{
 *  \brief  Write out list of items given by iterators. By default, items
 *          are separated by commas. After the last item, there is no delimiter.
 *          The list is generated to current output stream.
 *  \param  out target output stream
 *  \param  first iterator to the source container, first item to write.
 *  \param  last iterator that refers to the past-the-end, the iterator
 *          won't be generated.
 *  \param  output unary functor to write or transform a value. In case the
 *          return type of the functor is void,
 *          operator that will be used to write out the item to
 *          the output stream or its return value will.
 *  \param  delimiter separates two items, default value is a comma with one
 *          space character.
 */
template<class InputIterator, class OutputFunctor>
inline typename std::enable_if<
    !std::is_same<std::string, typename std::remove_reference<OutputFunctor>::type>::value>::type
WriteList(std::ostream& out,
    InputIterator first,
    InputIterator last,
    ...
    const char* delimiter = ", ")
{
    while (first != last)
    {
        detail::OutputFunctorHelper::Write(out, first, std::forward<OutputFunctor>(output));
        if (++first != last)
            out << delimiter;
    }
}
template<class InputIterator, class OutputFunctor>
inline void WriteList(std::ostream& out,
    InputIterator first,
    InputIterator last,
    OutputFunctor&& output,
    const std::string& delimiter)
{
    WriteList(out, first, last, std::forward<OutputFunctor>(output), delimiter.c_str());
}
