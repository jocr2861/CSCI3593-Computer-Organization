...
#ifndef UTILITY_CHECK_CONDITION_H_
#define UTILITY_CHECK_CONDITION_H_
 ...
 */
inline std::ostream& CheckConditionHeader(std::ostream& os, const char* file, const int line)
{
...
public:
     ...
     */
    ExceptionStream(const char* file, const int line, const std::string& defaultError = std::string());
    /**
     *  \brief  Constructor.
     *  \param  file File name where exception has been raised.
     *  \param  line Line number where exception has been raised.
     ...
     */
    ExceptionStream(const char* file, const int line, const bool cond, const std::string& conditionError);
    /**
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class Exception>
inline ExceptionStream<Exception>&
ExceptionStream<Exception>::operator<<(std::ostream& (*f)(std::ostream&))
{
    m_Stream << f;
    return *this;
}
 ...
 */
#define THROW(exception) throw ::codasip::detail::ExceptionStream<exception>(__FILE__, __LINE__)
/**
 *  \def    THROW_ERROR
 *  \brief  Throws codasip::exceptions::BasicError exception with given message.
 */
#ifndef LLVM_NO_EXCEPTIONS
#    define THROW_ERROR() throw ::codasip::detail::ExceptionStream<>(__FILE__, __LINE__)
#else
#    define THROW_ERROR() ::codasip::detail::ExceptionStream<>(__FILE__, __LINE__)
#endif
/**
...
#define CHECK_NOT_NULL(pointer) \
    if (!pointer)               \
    THROW_DEFAULT_ERROR("Dereferencing NULL pointer '" #pointer "'.")
/**
 *  \def    CHECK_NOT_EMPTY
...
#define CHECK_NOT_EMPTY(c) \
    if ((c).empty())       \
    THROW_DEFAULT_ERROR("Missing informations, container '" #c "' is empty.")
...
#endif   // UTILITY_CHECK_CONDITION_H_
