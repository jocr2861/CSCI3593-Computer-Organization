...
public:
  ...
  Symbol *FindFirstSymbolWithNameAndType(ConstString name,
                                         ...
                                         Visibility symbol_visibility);
  /// ==== CODASIP ====
  /// Expecting symbol address space dwarf (program or data dwarf), for CodAL nothing.
  Symbol *FindSymbolAtFileAddress(const codasip::AddressBase& file_addr);
  /// Expecting symbol address space dwarf (program or data dwarf), for CodAL nothing.
  Symbol *FindSymbolContainingFileAddress(const codasip::AddressBase& file_addr);
  /// Expecting symbol address space dwarf (program or data dwarf), for CodAL nothing.
  void ForEachSymbolContainingFileAddress(
      const codasip::AddressBase& file_addr, std::function<bool(Symbol *)> const &callback);
  /// =================
...
protected:
  ...
  /// ==== CODASIP ====
  typedef std::map<std::string, FileRangeToIndexMap*> AsToFileRangeToIndexMap;
  AsToFileRangeToIndexMap m_as_to_file_addr_to_index;
  /// =================
