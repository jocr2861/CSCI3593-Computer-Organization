...
/// \see Process::Notifications
  ...
  /// ==== CODASIP ====
  /**
   * \brief Reset the process and it's target
   */
  virtual Status Reset() { return Status(); }
  virtual Status GetCycleCount(uint64_t& cc) { return Status(lldb::eErrorTypeGeneric); };
  virtual DynamicRegisterInfo* GetDynamicRegisterInfo() {
    return nullptr;
  };
  virtual Status DumpProfiler() { return Status(lldb::eErrorTypeGeneric); }
  /// =================
...
public:
  ...
  /// ==== CODASIP ====
  virtual size_t DoReadMemory(codasip::AddressBase vm_addr, void *buf, size_t size,
                              Status &error) = 0;
  /// =================
  ...
  /// ==== CODASIP ====
  virtual size_t ReadMemory(codasip::AddressBase vm_addr, void *buf, size_t size,
                            Status &error);
  ...
  ///     returned in the case of an error.
  /// ==== CODASIP ====  
  size_t ReadMemoryFromInferior(codasip::AddressBase, void *buf, size_t size,
                                Status &error);
  /// =================
  ...
  /// ==== CODASIP ====
  size_t ReadStringFromMemory(codasip::AddressBase vm_addr, char *str, size_t max_bytes,
  ...
  /// ==== CODASIP ====
  size_t ReadCStringFromMemory(codasip::AddressBase vm_addr, char *cstr,
                               size_t cstr_max_len, Status &error);

  size_t ReadCStringFromMemory(codasip::AddressBase vm_addr, std::string &out_str,
                               Status &error);
  /// =================
  ...
  ///     order.
  /// ==== CODASIP ====
  uint64_t ReadUnsignedIntegerFromMemory(codasip::AddressBase load_addr,
                                         size_t byte_size, uint64_t fail_value,
                                         Status &error);

  int64_t ReadSignedIntegerFromMemory(codasip::AddressBase load_addr, size_t byte_size,
                                      int64_t fail_value, Status &error);

  lldb::addr_t ReadPointerFromMemory(codasip::AddressBase vm_addr, Status &error);

  bool WritePointerToMemory(codasip::AddressBase vm_addr, lldb::addr_t ptr_value,
                            Status &error);
  /// =================
  ...
  /// ==== CODASIP ====
  virtual size_t DoWriteMemory(codasip::AddressBase vm_addr, const void *buf,
  ...
  ///     The number of bytes that were actually written.
  /// ==== CODASIP ====
  size_t WriteScalarToMemory(codasip::AddressBase vm_addr, const Scalar &scalar,
                             size_t size, Status &error);
  size_t ReadScalarIntegerFromMemory(codasip::AddressBase addr, uint32_t byte_size,
  ...
  /// ==== CODASIP ====
  size_t WriteMemory(codasip::AddressBase vm_addr, const void *buf, size_t size,
                     Status &error);
  /// =================
  ...
  /// ==== CODASIP ====
  //------------------------------------------------------------------
  // Type for holding the list of addresses in a codal stack trace.
  //------------------------------------------------------------------
  typedef std::vector<lldb::addr_t> CodalStackTrace;
  //------------------------------------------------------------------
  // Get codal stack trace information from target.
  // No-op if not a codasip target or codal debugging is not supported.
  //------------------------------------------------------------------
  virtual CodalStackTrace GetCodalStackTrace() { return CodalStackTrace(); }
  //------------------------------------------------------------------
  // Set simulator verbosity
  //------------------------------------------------------------------
  virtual void SetCodasipSimulatorVerbosity(const llvm::StringRef verbosity) { /* no action*/ }
  /// =================
...
protected:
  ...
  void PrintWarning(uint64_t warning_type, const void *repeat_key,
                    /// ==== CODASIP ====
                    // format(printf) => format(gnu_printf)
  ...
  size_t RemoveBreakpointOpcodesFromBuffer(const codasip::AddressBase& addr, size_t size,
                                           uint8_t *buf) const;
  // =================
...
protected:
  ...
  GetStateChangedEventsPrivate(lldb::EventSP &event_sp,
                               const Timeout<std::micro> &timeout);
  /// ==== CODASIP ====
  size_t WriteMemoryPrivate(codasip::AddressBase addr, const void *buf, size_t size,
                            Status &error);
  /// =================
