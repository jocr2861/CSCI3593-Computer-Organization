...
public:
  ...
  // ==== CODASIP ====
  void SetCodasipSimulatorPath(const FileSpec &sim_path);

  const FileSpec &GetCodasipSimulatorPath() const;

  void SetCodasipSimulatorArgs(const Args &args);

  const Args &GetCodasipSimulatorArgs() const;
  Args &GetCodasipSimulatorArgs();

  void SetNamedPipe(const std::string &named_pipe);
  const std::string &GetNamedPipe() const;
...
protected:
  ...
  // ==== CODASIP ====
  FileSpec m_codasip_sim_path;
  Args m_codasip_sim_args;
  std::string m_named_pipe;
  // =================
  FileSpec m_shell;
