...
public:
  ...
  /// ==== CODASIP ====
  /// Queues the plan used to step through a codal range, stepping over
  /// codal function calls.
  ///
  /// @param[in] abort_other_plans
  ///    \b true if we discard the currently queued plans and replace them with
  ///    this one.
  ...
  ///     A shared pointer to the newly queued thread plan, or nullptr if the
  ///     plan could not be queued.
  virtual lldb::ThreadPlanSP QueueThreadPlanForStepOverCodalRange(
      bool abort_other_plans,
      lldb::RunMode stop_other_threads);

  /// Queues the plan used to step one cycle
  ///
  ///
  /// @param[in] stop_other_threads
  ///    \b true if we will stop other threads while we single step this one.
  ...
  ///     A shared pointer to the newly queued thread plan, or nullptr if the
  ///     plan could not be queued.
  virtual lldb::ThreadPlanSP QueueThreadPlanForStepCycle(
      bool abort_other_plans,
...
/// ==== CODASIP ====
  void SetStepInCodalToPlan(lldb::ThreadPlanSP& plan, const StepType stepType);
/// =================
protected:
