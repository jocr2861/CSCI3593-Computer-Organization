...
class Address : public codasip::AddressBase {
public:
  ...
  //------------------------------------------------------------------
  /// ==== CODASIP ====
  Address() : AddressBase(LLDB_INVALID_ADDRESS), m_section_wp() {}
  /// =================
  /// Copy constructor
  ///
  ...
  Address(const Address &rhs)
      : AddressBase(rhs), m_section_wp(rhs.m_section_wp) {}
  /// =================
                       ...
                       // invalid (the weak_ptr will throw)
  {
    ...
    /// ==== CODASIP ====
    if (section_sp) // set address space from section
      SetDwarf(section_sp->GetAddressSpace().GetDwarf());
    /// =================
  }

  explicit Address(const codasip::AddressBase& addr)
     : AddressBase(addr)
  {}
  /// =================
  ...
  /// ==== CODASIP ====
  explicit Address(lldb::addr_t file_addr, const uint32_t dwarf);
  /// =================
...
///     A const Address object reference to \a this.
  ...
  /// ==== CODASIP ====
  codasip::AddressBase GetLoadAddress(Target *target) const;
  /// =================
  ...
  ///     not loaded.
  /// ==== CODASIP ====
  codasip::AddressBase GetOpcodeLoadAddress(
  /// =================
      Target *target,
  ...
  /// ==== CODASIP ====
  // addr_t -> AddressBase
  bool SetLoadAddress(codasip::AddressBase load_addr, Target *target,
                      bool allow_section_end = false);

  bool SetOpcodeLoadAddress(
      codasip::AddressBase load_addr, Target *target,
  ...
  void SetSection(const lldb::SectionSP &section_sp) {
    m_section_wp = section_sp;
    /// ==== CODASIP ====
    // TODO: set always, or only if current AS/dwarf index is invalid?
    if (section_sp)
      SetDwarf(section_sp->GetAddressSpace().GetDwarf());
    /// =================
  }
  ...
  /// ==== CODASIP ====
  uint32_t GetDwarf() const override;
  virtual void SetDwarf(uint32_t dwarf) override;
  virtual void SetDwarf(const codasip::AddressSpace& as) override;
  /// =================
...
protected:
  ...
  /// ==== CODASIP ====
  // We have AddressBase instead of offset
