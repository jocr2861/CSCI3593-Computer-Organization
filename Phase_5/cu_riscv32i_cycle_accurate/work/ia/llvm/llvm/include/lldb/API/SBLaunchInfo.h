...
public:
  ...
  // ==== CODASIP ====
  void SetCodasipSimulatorPath(const char *sim_path);

  const char *GetCodasipSimulatorPath() const;

  void SetCodasipSimulatorArgs(const char **argv, bool append);

  void SetNamedPipe(const char *named_pipe);

  const char *GetNamedPipe() const;

  // CodasipSimulatorPlatform modifies the internal ProcessLaunchInfo object
  // Use Backup() and Restore() if ProcessLaunch() is called multiple times
  // with the same arguments to avoid side effects.
  void Backup();
  void Restore();
  // =================
