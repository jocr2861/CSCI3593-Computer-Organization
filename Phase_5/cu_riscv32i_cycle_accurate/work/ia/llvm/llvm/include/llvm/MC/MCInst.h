...
class MCOperand {
  ...
  // ==== CODASIP ====
  /// Attribute identifier, empty when not set
...
// ==== CODASIP ====
  static MCOperand createImm(int64_t Val, const bool IsMasked) {
    MCOperand Op = createImm(Val);
    Op.IsMasked = IsMasked;
    return Op;
  }
  const std::string& getAttributeId() const { return AttributeId; };
  void setAttributeId(const std::string& Val) { AttributeId = Val; };
