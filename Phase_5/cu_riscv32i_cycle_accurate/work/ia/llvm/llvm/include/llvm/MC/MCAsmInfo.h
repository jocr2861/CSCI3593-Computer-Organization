...
protected:
  ...
  // ==== CODASIP ====
  // Only place which worked is here for address spaces, not in MCContext or TargetStreamer
public:
  typedef SmallVector<CodasipAddressSpace, 3> AddressSpacesVec;
  const AddressSpacesVec &getAddressSpaces() const { return AddressSpaces; }
  static unsigned int getOctetsCount(unsigned int ByteBitWidth) {
    unsigned int ByteOctetsSize = 1;
    while (ByteBitWidth > 8) {
      ByteOctetsSize *= 2;
      if (ByteBitWidth % 2) {
        ByteBitWidth /= 2;
        ByteBitWidth++;
    ...
    return ByteOctetsSize;
  }
protected:
  /// Stored address spaces, has to be static to be shared between different contexts
  AddressSpacesVec AddressSpaces;
  // =================
