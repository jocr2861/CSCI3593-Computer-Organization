...
public:
  ...
  // ==== CODASIP ====
  /// Returns the disassembly of a single instruction.
  /// Usable only for codasip_disassembler built-in inside Codasip Simulator.
  ...
  /// \example          Multiple bit-widths Bytes '0xAAAAAAAA' and '0x0000BBBB', Bytes.size() = 4
  ///
  /// \param Instr    - An MCInst to populate with the contents of the
  ///                   instruction.
  /// \param Size     - A value to populate with the size of the instruction, or
  ...
  ///                                            disassemblable but invalid,
  ///                   MCDisassembler::Fail if the instruction was invalid.
  virtual DecodeStatus getInstructionBuiltin(MCInst &Instr, uint64_t &Size,
