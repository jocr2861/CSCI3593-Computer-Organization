...
public:
  ...
  Scalar(llvm::APInt v) : m_type(), m_float(static_cast<float>(0)) {
    ...
    /// ==== CODASIP ====
    default:
      if (m_integer.isSignedIntN(m_integer.getBitWidth()))
        m_type = e_sint_n;
      else
        m_type = e_uint_n;
      return;
  ...
  bool MakeUnsigned();
  /// ==== CODASIP ====
  const char *GetValueTypeAsCString(Scalar::Type value_type) const;
  /// =================
  ...
  /// ==== CODASIP ====
  llvm::APInt SIntN(const llvm::APInt &fail_value) const;

  llvm::APInt UIntN(const llvm::APInt &fail_value) const;

  static llvm::APInt ToIntN(const DataExtractor& data, const unsigned byte_size);
  /// =================
