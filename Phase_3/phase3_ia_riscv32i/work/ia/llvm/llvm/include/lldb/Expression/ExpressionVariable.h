...
public:
  ...
  /// ==== CODASIP ====
  codasip::Resource *GetResource() {
    return m_frozen_sp->GetValue().GetResource();
  }

  void SetResource(const Value::ContextType context_type, const void *resource) {
    return m_frozen_sp->GetValue().SetContext(context_type, const_cast<void *>(resource));
  }
  /// =================
