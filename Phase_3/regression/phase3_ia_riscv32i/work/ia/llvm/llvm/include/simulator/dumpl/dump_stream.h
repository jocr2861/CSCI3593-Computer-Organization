...
public:
    ...
    using IsIntegral =
        typename std::enable_if<std::is_integral<typename std::decay<T>::type>::value == Is>::type;
    ...
    template<class T>
    IsIntegral<T, true>
    DumpAddr(const std::string& id, T&& data, const Address addr, const unsigned dataBits);
    template<class T>
    IsIntegral<T, false>
    DumpAddr(const std::string& id, T&& data, const Address addr, const unsigned dataBits);
    /**
...
class DumpStreamIterator : public std::iterator<std::forward_iterator_tag,   // Forward iterator type
...
inline DumpStream::IsIntegral<T, true> DumpStream::Dump(const std::string& id, T&& data)
{
    ...
    stream << std::setfill('0') << std::setw(sizeof(T) * 2) << std::hex << +data;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class T>
inline DumpStream::IsIntegral<T, true>
DumpStream::DumpAddr(const std::string& id, T&& data, const Address addr, const unsigned dataBits)
{
    const auto mask = BitMask<typename std::decay<T>::type>(dataBits);

    std::stringstream stream;
    stream << std::setfill('0') << std::setw(((dataBits - 1) / 4) + 1) << std::hex << ((+data) & mask);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class T>
inline DumpStream::IsIntegral<T, false>
DumpStream::DumpAddr(const std::string& id, T&& data, const Address addr, const unsigned dataBits)
{
