...
#ifndef COMMON_CODASIP_INTEGER_DETAIL_DEFS_H_
#define COMMON_CODASIP_INTEGER_DETAIL_DEFS_H_
...
template<typename T>
struct IsIntegral
  : public std::integral_constant<bool, std::is_integral<typename UnderlyingTypeHelper<T>::type>::value>
{};
...
#    define _CODASIP_INT_ENABLE_IF(cnd, R) typename std::enable_if<cnd, R>::type
#else
...
#    define _CODASIP_INT_INTEGRAL_CND(T) (::codasip::integer::detail::IsIntegral<T>::value)
#else
...
#    define _CODASIP_INT_ENABLE_IF_POD(T, R) _CODASIP_INT_ENABLE_IF(_CODASIP_INT_INTEGRAL_CND(T), R)
#else
...
#    define _CODASIP_INT_VECTOR_SCALAR_ASSERTIONS(T1, T2, SIZE)                       \
        static_assert(_CODASIP_INT_INTEGRAL_CND(T1) && _CODASIP_INT_INTEGRAL_CND(T2), \
            "Vector operations are defined for integral types only.");                \
        static_assert((SIZE != 0), "Vector operations are defined for non-zero vector lengths.")
#else
#    error "Macro _CODASIP_INT_VECTOR_SCALAR_ASSERTIONS already defined!!!"
#endif

#endif   // COMMON_CODASIP_INTEGER_DETAIL_DEFS_H_
