...
//
// author     : Tomas Minac, Ondrej Glasnak
// maintainer : glasnak@codasip.com
// about      : inspired by ScheduleDAGSuperBlock.h    // how motivating
...
protected:
  ...
  /// physRegContainUnitVec - is set of instructions, which contain
  ...
  unsigned iterateChainSucc(AliasAnalysis *AA, const MachineFrameInfo *MFI,
                            SUnit *SUa, SUnit *SUb, SUnit *ExitSU,
                            unsigned *Depth,
  ...
  /// of some BasicBlock
  bool isInExitSUs(const SUnit *su) const { return is_contained(ExitSUs, su); }

  /// isAlreadyConnected - method, which return, if succ is successor
  /// of predecessor.
  bool isAlreadyConnected(SUnit *succ, SUnit *pred) {
    for (auto PredOfSucc : succ->Preds) {
      SUnit *PredSU = PredOfSucc.getSUnit();
      if (PredSU == pred) {
        return true;
      }
    }
  ...
  bool isSchedulingBoundary(const MachineInstr *MI) const {
    // Terminators, labels and calls can't be scheduled
    if (MI->isTerminator() || MI->isLabel() || MI->isCall()) {
      return true;
    }
    return false;
  }
...
public:
  ...
  virtual void enterRegion(MachineBasicBlock *bb,
                           ...
                           MachineBasicBlock::iterator end, unsigned endcount);
...
#endif
  SUnits.emplace_back(SUnit(MI, (unsigned)SUnits.size()));
...
/// getSUnit - Return an existing SUnit for this MI, or nullptr.
