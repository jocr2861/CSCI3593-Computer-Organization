...
#ifndef COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_ARITHMETIC_H_
#define COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_ARITHMETIC_H_
...
{
    typedef typename underlying_type_helper<native_int_bitwidth_helper<T>::value +
            native_int_bitwidth_helper<uint8_t>::value,
 ...
 */
template<template<class> class Binary, unsigned MIN_SIZE, unsigned LHS_SIZE, typename T1, typename T2, typename DoubleT>
_CODASIP_INT_INLINE void DoAdditivePropagateSignAndCarry(T1* lhs, const T2 sign, DoubleT carry)
{
    ...
    {
        carry += Binary<double_UT_type>()(static_cast<UT>(lhs[ii]), static_cast<UT>(sign));
 ...
 */
template<template<class> class Binary, unsigned LHS_SIZE, unsigned RHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE void DoAdditive(T1* lhs, const T2* rhs)
{
    ...
    {
        carry += Binary<double_UT_type>()(static_cast<UT>(lhs[ii]), static_cast<UT>(rhs[ii]));
...
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((sizeof(T1) >= sizeof(T2)), void) DoAdditive(T1* lhs, const T2 rhs)
{
    ...
    double_ST_type carry = Binary<double_ST_type>()(static_cast<UT>(lhs[0]), static_cast<UT>(rhs));
    lhs[0] = static_cast<T1>(carry);
    carry >>= native_int_bitwidth_helper<T1>::value;
    DoAdditivePropagateSignAndCarry<std::plus, 1, LHS_SIZE>(lhs, GetSignExtension(rhs), carry);
}
...
{   // cast it to shorter type and look at it as array
    ...
    {
        carry += Binary<double_ST_type>()(static_cast<UT>(lhs[ii]), static_cast<UT>(value));
        lhs[ii] = static_cast<T1>(carry);
        carry >>= native_int_bitwidth_helper<T1>::value;
        value >>= native_int_bitwidth_helper<T1>::value;
    }
    DoAdditivePropagateSignAndCarry<Binary, MIN_SIZE, LHS_SIZE>(lhs, GetSignExtension(value), carry);
}
...
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF_POD(T2, void) DoBinaryPlus(T1* lhs, const T2 rhs)
{
 ...
 */
template<unsigned LHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF_POD(T2, void) DoBinaryMinus(T1* lhs, const T2 rhs)
{
...
DoMultiplicationUnsignedRound(T1* result, T1* tmp, const T1* lhs, const T2 rhs, const unsigned shiftOffset)
{
    T2 rhsValue = rhs;
 ...
 */
template<unsigned LHS_SIZE, unsigned RHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE void DoBinaryMultiplication(T1* lhs, const T2* rhs)
{
    _CODASIP_INT_VECTOR_ASSERTIONS(T1, T2);
    ...
    {
        DoMultiplicationUnsignedRound<LHS_SIZE>(result,
            tmp,
            lhs,
            rhs[ii],
...
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF((LHS_SIZE == 1), void) DoUnaryNeg(T* lhs)
{
...
#endif   // COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_ARITHMETIC_H_
