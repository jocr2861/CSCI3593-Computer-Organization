...
#ifndef COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_COMPARISON_H_
#define COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_COMPARISON_H_
...
_CODASIP_INT_INLINE void CopyAndExtend(T1* dst, const T2* src)
{
    ...
    std::memset(dst + MIN_SIZE, GetSignExtension(src[MIN_SIZE - 1]), (MAX_SIZE - MIN_SIZE) * sizeof(T1));
}
...
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF_POD(T, int) DoCompareRaw(T* lhs, const T* rhs)
{
...
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF((sizeof(T1) == sizeof(T2)) && (LHS_SIZE == RHS_SIZE), bool)
...
template<unsigned LHS_SIZE, unsigned RHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((LHS_SIZE > RHS_SIZE), bool) DoEqual(const T1* lhs, const T2* rhs)
{
    _CODASIP_INT_VECTOR_ASSERTIONS(T1, T2);
    ...
    return DoEqualSign<MIN_SIZE, MAX_SIZE>(lhs, GetSignExtension(rhs[RHS_SIZE - 1]));
}
...
_CODASIP_INT_INLINE
_CODASIP_INT_ENABLE_IF((sizeof(T1) >= sizeof(T2)), bool) DoEqual(const T1* lhs, const T2 rhs)
{
...
template<unsigned LHS_SIZE, unsigned RHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE int DoCompare(const T1* lhs, const T2* rhs)
{
    _CODASIP_INT_VECTOR_ASSERTIONS(T1, T2);

    enum : unsigned
    {
        SIZE = static_max_helper<LHS_SIZE, RHS_SIZE>::value + 1
    };

    typedef typename integer::detail::BinaryResultMaxPod<native_int_bitwidth_helper<T1>::value,
...
template<unsigned LHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF_POD(T2, int) DoCompare(const T1* lhs, const T2 rhs)
{
    _CODASIP_INT_VECTOR_SCALAR_ASSERTIONS(T1, T2, LHS_SIZE);

    enum : unsigned
    {   // check whether lhs must be extended
        SIZE = (((LHS_SIZE * sizeof(T1)) >= sizeof(T2)) ? LHS_SIZE : (sizeof(T2) / sizeof(T1))) + 1
    };
    typedef typename integer::detail::BinaryResultMaxPod<native_int_bitwidth_helper<T1>::value,
        std::is_signed<T1>::value,
        native_int_bitwidth_helper<T1>::value,
...
template<unsigned LHS_SIZE, unsigned RHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE int DoLess(const T1* lhs, const T2* rhs)
{
...
template<unsigned LHS_SIZE, unsigned RHS_SIZE, typename T1, typename T2>
_CODASIP_INT_INLINE int DoGreater(const T1* lhs, const T2* rhs)
{
    return DoCompare<LHS_SIZE, RHS_SIZE>(lhs, rhs) > 0;
}
...
#endif   // COMMON_CODASIP_INTEGER_FIXED_ARRAY_BACKEND_COMPARISON_H_
