...
#ifndef COMMON_CODASIP_INTEGER_INTEGER_NUMBER_BINARY_OPS_H_
#define COMMON_CODASIP_INTEGER_INTEGER_NUMBER_BINARY_OPS_H_

#include "common/codasip_integer/integer_number.h"

#ifndef _CODASIP_INT_RESULT_TYPE
#    define _CODASIP_INT_RESULT_TYPE(result) \
        typename codasip::integer::detail::  \
            BinaryOperatorHelper<codasip::integer::IntegerNumber, LHS_BITS, LHS_SIGN, RHS_BITS, RHS_SIGN>::result
#else
...
#    define _CODASIP_INT_RESULT_TYPE_POD(pod_type, result) \
        typename codasip::integer::detail::                \
            BinaryResultNative<codasip::integer::IntegerNumber, _CODASIP_BITS, _CODASIP_SIGN, pod_type>::result
#else
...
#    define _CODASIP_INT_ENABLE_RESULT_IF_POD(pod_type, result)      \
        typename std::enable_if<_CODASIP_INT_INTEGRAL_CND(pod_type), \
            _CODASIP_INT_RESULT_TYPE_POD(pod_type, result)>::type
#else
...
#    define _CODASIP_INT_ENABLE_MOVE_RESULT_IF(result)                                           \
        typename std::enable_if<std::is_same<_CODASIP_INT_RESULT_TYPE(result),                   \
                                    codasip::integer::IntegerNumber<LHS_BITS, LHS_SIGN>>::value, \
            _CODASIP_INT_RESULT_TYPE(result)>::type
#else
...
#    define _CODASIP_INT_ENABLE_MOVE_RESULT_IF_POD(pod_type, result)                       \
        typename std::enable_if<_CODASIP_INT_INTEGRAL_CND(pod_type) &&                     \
                std::is_same<_CODASIP_INT_RESULT_TYPE_POD(pod_type, result),               \
                    codasip::integer::IntegerNumber<_CODASIP_BITS, _CODASIP_SIGN>>::value, \
            _CODASIP_INT_RESULT_TYPE_POD(pod_type, result)>::type
#else
...
#    define _CODASIP_INT_DEF_BINARY(op, result)                                                      \
        ...
        {                                                                                            \
            ...
            return std::move(lhs);                                                                   \
                                                                                                     ...
                                                                                                     \
        template<unsigned _CODASIP_BITS, bool _CODASIP_SIGN, typename T>                             \
        _CODASIP_INT_INLINE _CODASIP_INT_ENABLE_RESULT_IF_POD(T, result) operator op(const T lhs,    \
        ...
        _CODASIP_INT_INLINE _CODASIP_INT_ENABLE_MOVE_RESULT_IF(result) operator op(                  \
            codasip::integer::IntegerNumber<LHS_BITS, LHS_SIGN>&& lhs,                               \
        ...
        _CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF(                                                  \
            (codasip::integer::IntegerNumber<LHS_BITS, LHS_SIGN>::IS_WRAPPER ==                      \
                ...
                (codasip::integer::IntegerNumber<LHS_BITS, LHS_SIGN>::IS_WRAPPER),                   \
            _CODASIP_INT_RESULT_TYPE(result))                                                        \
        ...
        {                                                                                            \
            typedef _CODASIP_INT_RESULT_TYPE(result) result_type;                                    \
            typedef                                                                                  \
                typename codasip::integer::IntegerNumber<RHS_BITS, RHS_SIGN>::pod_type rhs_pod_type; \
            result_type nrv(lhs);                                                                    \
...
#    define _CODASIP_INT_DEF_BINARY_SHIFT(op)                                                  \
        ...
        {                                                                                      \
            ...
            return std::move(lhs);                                                             \
                                                                                               ...
                                                                                               \
        template<unsigned _CODASIP_BITS, bool _CODASIP_SIGN, typename T>                       \
        _CODASIP_INT_INLINE _CODASIP_INT_ENABLE_IF_POD(T, T) operator op(const T lhs,          \
            const codasip::integer::IntegerNumber<_CODASIP_BITS, _CODASIP_SIGN>& rhs)          \
        {                                                                                      \
            return lhs op static_cast<const unsigned>(rhs);                                    \
                                                                                               ...
                                                                                               \
        template<unsigned LHS_BITS, bool LHS_SIGN, unsigned RHS_BITS, bool RHS_SIGN>           \
        _CODASIP_INT_INLINE _CODASIP_INT_RESULT_TYPE(shift_type) operator op(                  \
            const codasip::integer::IntegerNumber<LHS_BITS, LHS_SIGN>& lhs,                    \
            const codasip::integer::IntegerNumber<RHS_BITS, RHS_SIGN>& rhs)                    \
        {                                                                                      \
        ...
        template<unsigned LHS_BITS, bool LHS_SIGN, unsigned RHS_BITS, bool RHS_SIGN>           \
        _CODASIP_INT_INLINE _CODASIP_INT_ENABLE_MOVE_RESULT_IF(shift_type) operator op(        \
            codasip::integer::IntegerNumber<LHS_BITS, LHS_SIGN>&& lhs,                         \
            const codasip::integer::IntegerNumber<RHS_BITS, RHS_SIGN>& rhs)                    \
        {                                                                                      \
            lhs op## = static_cast<const unsigned>(rhs);                                       \
...
#endif   // COMMON_CODASIP_INTEGER_INTEGER_NUMBER_BINARY_OPS_H_
