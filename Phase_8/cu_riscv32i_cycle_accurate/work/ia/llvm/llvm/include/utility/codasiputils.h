...
#ifndef UTILITY_CODASIP_UTILS_H_
#define UTILITY_CODASIP_UTILS_H_
 ...
 *  \}
 */
#define BIND_THIS(x) std::bind1st(std::mem_fun(&THIS_CLASS::x), this)
 ...
 */
/**
 *  \brief  Auxiliary function to generate spaces (indent) for DebugPrints
 ...
 */
std::string getCType(const unsigned int pSize, const bool pSigned, const bool useBool);

/**
 * \brief   Used for instances and attributes for optimizations reasons (the
 *          bit-width is assured by the simulation engine).
 */
std::string getSTDCType(const unsigned int pSize, const bool pSigned, const bool useBool);
 ...
 */
std::string
GetCArithmeticType(const bool isReal, const int bitWidth, const bool isSigned, const bool useBool);
...
/**
 * \brief Returns the result of `std::set_difference` in a new set instance.
 * \param a The LHS.
 * \param b The RHS.
 */
template<typename T>
std::set<T> MakeSetDifference(const std::set<T>& a, const std::set<T>& b)
{
    std::set<T> diff;
 ...
 */
template<typename T>
inline T RangeSize(const T left, const T right)
{
...
/**
 *  \brief  Check whether given value is in specified range or not (both range
 *          borders are counted to be in range).
 *  \param  value target value to check
 *  \param  left first range border
 *  \param  right second range border
 */
template<typename T1, typename T2>
inline bool IsInRange(const T1& value, const T2& left, const T2& right)
{
    return (static_cast<T1>(std::min(left, right)) <= value) &&
        (value <= static_cast<T1>(std::max(left, right)));
}
 ...
 */
FORCE_INLINE constexpr bool IsPowerOfTwo(size_t value)
{
    return ((value != 0) && ((value & (~value + 1)) == value));
}

class NextClosestPowTwo
{
public:
    /**
     *  \brief  Get next closest power of two, 32 -> 32, 45 -> 64, 21 -> 32
     *  \param[in] value to find closest power of two to
     *  \return closest power of two
     */
    constexpr static size_t Get(size_t input)
    {
        return OrRot(OrRot(OrRot(OrRot(OrRot(input - 1, 1), 2), 4), 8), 16) + 1;
    }

private:
    constexpr static size_t OrRot(size_t input, size_t rotation)
    {
        return input | (input >> rotation);
    }
};
 ...
 */
template<typename T>
inline T* CopyPtr(const T* const ptr)
{
...
{
    template<typename T>
    void operator()(const T* const ptr) const
    {
        delete ptr;
    }
};
 ...
 */
template<typename T, int BITS, bool SIGNED>
struct TypeBitWidth<T, integer::IntegerNumber<BITS, SIGNED>>
{
    enum
    {
        value = integer::IntegerNumber<BITS, SIGNED>::BIT_WIDTH
    };
};
...
{
    return length ? codasip::MaxUint(-1) >> (MaxUint::BIT_WIDTH - length) : codasip::MaxUint(0);
}
 ...
 */
template<typename IntegerType>
void GetRange(IntegerType& a, IntegerType& b, const std::string& source, const std::string& delimiter = "..")
{
    // default values
    a = 0;
    b = 0;
...
#endif   // UTILITY_CODASIP_UTILS_H_
