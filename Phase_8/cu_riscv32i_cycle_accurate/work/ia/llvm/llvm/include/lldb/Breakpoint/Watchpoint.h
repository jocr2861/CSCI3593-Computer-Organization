...
public:
  ...
  /// ==== CODASIP ====
  // need to return reference to avoid memory failures in SB API
  const std::string& GetWatchSpec() const;
  /// =================
  void SetWatchSpec(const std::string &str);
  /// ==== CODASIP ====
  void SetWatchpointData(const llvm::APInt& data);
  const llvm::APInt* GetWatchpointData() const;
  std::string GetWatchpointDataStr() const;
  lldb::ValueObjectSP GetOldValue() const { return m_old_value_sp; }
  lldb::ValueObjectSP GetNewValue() const { return m_new_value_sp; }
  /// =================
