...
namespace llvm {
  ...
  private:
    ...
    // ==== CODASIP ====
  public:
    int getCurrentDataAddressSpaceIndex() const { return CurrentDataAddressSpaceIndex; }
    void setCurrentDataAddressSpaceIndex(int value) { CurrentDataAddressSpaceIndex = value; }

  private:
    int determineDefaultAddressSpaceIndex(StringRef Id, unsigned Type, unsigned Flags) const;

    /// Current data address space index, created data sections should be enhanced by it
    int CurrentDataAddressSpaceIndex = 0;
    // =================
