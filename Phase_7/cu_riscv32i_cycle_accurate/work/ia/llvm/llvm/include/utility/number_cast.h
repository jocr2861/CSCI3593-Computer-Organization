...
#ifndef UTILITY_NUMBER_CAST_H_
#define UTILITY_NUMBER_CAST_H_
 ...
 */
inline void throw_invalid_arg(const std::string& error_message, const char* file, const int line)
{
 ...
 */
#define CODASIP_NC_THROW_INVALID_CAST(message) throw_invalid_arg((message), __FILE__, __LINE__)
...
private:
    ...
    static int HandleCharConstant(const std::string& value)
    {
        ...
        if ((std::count(string_value.begin(), string_value.end(), '\\') > 1) && (string_value != "\\\\"))
            CODASIP_NC_THROW_INVALID_CAST(ERR_CHAR_CONV + " '" + value + "'");
        // if it is escape sequence get it's value, otherwise simple access
        return ((string_value[0] == '\\') ? EscapeSequenceValue(string_value) : string_value[0]);
    }
    /**
    ...
    static int EscapeSequenceValue(const std::string& string_value)
    {
        ...
        static const size_t simple_escape_sequence_count = sizeof(simple_escape_sequence) /
            sizeof(escape_pair);
        ...
        if (string_value.size() < 2 || string_value[0] != '\\')
            CODASIP_NC_THROW_INVALID_CAST(ERR_ESCAPE_CHAR_CONV + "'" + string_value + "'");
        ...
        {   // check underflow/overflow/out of range
            CODASIP_NC_THROW_INVALID_CAST(ERR_BAD_ESCAPE_CHAR + "'" + string_value + "'");
        }
    ...
    static int CheckNumberSuffix(std::string& str_value)
    {
        ...
        for (iterator rit = value.rbegin(); rit != value.rend(); ++rit)
        {
            ...
            {   // unsupported character, probably bad number format
                CODASIP_NC_THROW_INVALID_CAST(ERR_UNSUPPORTED_DIGIT + "'" + value + "'");
            }
        }
        ...
        str_value = value;
        return ((lcount == 2) ? std::numeric_limits<unsigned long long>::digits
    ...
    static int CheckRealNumberSuffix(std::string& str_value)
    {
        ...
        str_value = value;
        return (fcount) ? FLOAT_BIT_WIDTH : (lcount) ? LONG_DOUBLE_BIT_WIDTH : DOUBLE_BIT_WIDTH;
    }
};
...
inline std::string
IntToStr(const IntegerType& value, std::ios_base& (*number_base)(std::ios_base&) = std::dec)
{
 ...
 *  \warning Do not use codasip_int as IntegerType
...
template<>
inline long double StrToReal<long double>(const std::string& value, int& bitwidth)
{
