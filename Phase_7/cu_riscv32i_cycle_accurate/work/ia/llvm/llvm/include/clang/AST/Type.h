...
public:
  ...
  // === CODASIP ===
  bool isVariableBitWidthIntType() const;
  // ===============
...
// ========================= CODASIP ==========================================
class VariableBitWidthIntType : public Type {
private:
  unsigned Width;
  bool Signed;
  QualType elemType;
  VariableBitWidthIntType(unsigned W, bool S, QualType Element, QualType CanonicalPtr)
  : Type(VariableBitWidthInt,
		 CanonicalPtr /*QualType()*/,
		 /*dependent*/ /*false*/Element->isDependentType() ,
		 /*instantion dependent*/ Element->isInstantiationDependentType(),
		 /*Variably modified*/ Element->isVariablyModifiedType(),
		 /*ContainsUnexpandedParameterPack*/ Element->containsUnexpandedParameterPack()
	),
    Width(W), Signed(S) , elemType(Element) { }
  friend class ASTContext;
public:
  QualType getElementType() const { return elemType; }
  unsigned getWidth() const {return Width;}
  bool isSigned() const {return Signed;}
  // QualType getBaseQualType() const {return baseType;}
  bool isSugared() const {return false;}
  QualType desugar() const {return QualType(this,0); }
  static bool classof(const Type *T) {return T->getTypeClass() == VariableBitWidthInt; }
  static bool classof(const VariableBitWidthIntType *) { return true; }
  // Returns bitwidth of a larger data type, e.g. 8 for 5, 16 for 12, 64 for 33, etc
  // In fact it returns the size that should be used in memory.
  static uint64_t getLargerStdTypeBitWidth(const uint64_t OrigBitWidth);
  // Returns byte size of a larger data type, e.g. 4 for 3, 8 for 5, etc.
  // In fact it returns the byte size that should be used in memory.
  // Similar to getLargerStdTypeBitWidth.
  static CharUnits getLargerStdTypeByteSize(const CharUnits& OrigChars);
};
// ============================================================================
...
// =============== CODASIP ===============
inline bool Type::isVariableBitWidthIntType() const {
  return isa<VariableBitWidthIntType>(CanonicalType);
}
// =======================================
...
inline bool Type::isIntegerType() const {
  ...
  // =============== CODASIP =============================================
  if (isa<VariableBitWidthIntType>(CanonicalType))
    return true;
  // ============================================================
  return false;
}
