...
#ifndef OS_FILESYSTEML_CODASIP_IO_H
#define OS_FILESYSTEML_CODASIP_IO_H
...
public:
    ...
    */
     ...
     */
    explicit Fstream(const filesystem::Path& file, mode m = in | out)
      : std::fstream()
    {
        Open(file, m);
    }
...
public:
    ...
    */
     ...
     * \param m Mode used for opening stream for \p file (in, out, app, etc.).
     */
    explicit Ifstream(const filesystem::Path& file, mode m = in)
      : std::ifstream()
    {
        Open(file, m);
    }
...
public:
    ...
    */
     ...
     * \param m Mode used for opening stream for \p file (in, out, app, etc.).
     */
    explicit Ofstream(const filesystem::Path& file, mode m = out | trunc)
      : std::ofstream()
    {
        Open(file, m);
    }
...
#endif /* OS_FILESYSTEML_CODASIP_IO_H */
