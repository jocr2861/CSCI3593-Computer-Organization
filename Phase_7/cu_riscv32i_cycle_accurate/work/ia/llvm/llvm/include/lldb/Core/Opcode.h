...
public:
  ...
  /// ==== CODASIP ====
  size_t GetTargetByteSize() const { return m_target_byte_size; }
  /// =================
  ...
  void SetOpcodeBytes(const void *bytes, size_t length,
                      lldb::ByteOrder order = endian::InlHostByteOrder(),
...
protected:
  ...
  union {
    ...
    struct {
      /// ==== CODASIP ====
      uint8_t bytes[64]; // This must be big enough to handle any opcode for any
                         // supported target.
      /// =================
      uint8_t length;
    } inst;
